...
augroup fileExplorer
  au!
  au BufEnter *.exp let oldSwap=&swapfile | set noswapfile
  au BufLeave *.exp let &swapfile=oldSwap
"  au BufEnter _fileExplorer.exp nm <cr> :call ExplProcessFile(ExplGetFileName())<cr>
  au BufEnter *.exp nmap <cr> :call ExplProcessFile(ExplGetFileName())<cr>
  au BufLeave *.exp nun <cr>
  au BufEnter *.exp nm - :call ExplProcessFile(g:currDir."../")<cr>
  au BufLeave *.exp nun -
  au BufEnter *.exp nm c :ChangeDirectory to: 
  au BufLeave *.exp nun c
  au BufEnter *.exp nm r :call ExplRenameFile()<cr>
  au BufLeave *.exp nun r
  au BufEnter *.exp nm d :. call ExplDeleteFile()<cr>
  au BufLeave *.exp nun d
  au BufEnter *.exp vm d :call ExplDeleteFile()<cr>
  au BufLeave *.exp vun d
  au BufEnter *.exp nm q :call ExplBack2PrevFile()<cr>
  au BufLeave *.exp nun q
  au BufEnter *.exp nm s :exec ("cd ".escape(g:currDir,' '))<cr>
  au BufLeave *.exp nun s
  au BufEnter *.exp command! -nargs=+ -complete=dir ChangeDirectory call ExplGotoDir(<f-args>)
  au BufLeave *.exp delcommand ChangeDirectory
  au BufEnter * nested call ExplEditDir(expand("%"))
augroup end
 
function! ExplInitiate(split, ...)
  if (expand("%:p:t")=="_fileExplorer.exp")
...
function! ExplBack2PrevFile()
  if ((@#!="") && (@#!="_fileExplorer.exp") && (b:splitWindow==0) && 
        \(isdirectory(@#)==0))
    exec("e #")
  endif
  call ExplCloseExplorer()
