...
" $Id: cvscommand.vim,v 1.31 2002/11/21 00:41:55 rhiestan Exp $
"
" Vim plugin to assist in working with CVS-controlled files.
"
" Last Change:   $Date: 2002/11/21 00:41:55 $
...
" Section: Useful pattern constants 

let s:CVSCommandTagPattern='\a[A-Za-z0-9-_]*'

" Section: Utility functions 

" Function: s:CVSResolveLink() 
...
function! s:CVSEditFile(name, origBuffNR)
  let v:errmsg = ""
  let editCommand = s:CVSGetOption('CVSCommandEdit', 'split')
  if editCommand != 'edit' 
...
" Function: s:CVSDoCommand(cvscmd, cmdName, statusText) 
...
" Function: s:CVSGetStatusVars(revision, branch) 
"
...
function! s:CVSVimDiff(...)
  ...
  let g:CVSCommandRestoreVimDiffStateCmd =  "call setbufvar(".originalBuffer.", \"&diff\", ".getwinvar(originalWindow, '&diff').")" . "|call setbufvar(".originalBuffer.", \"&foldcolumn\", ".getwinvar(originalWindow, '&foldcolumn').")" . "|call setbufvar(".originalBuffer.", \"&foldmethod\", '". getwinvar(originalWindow, '&foldmethod')."')" . "|call setbufvar(".originalBuffer.", \"&scrollbind\", ".getwinvar(originalWindow, '&scrollbind').")" . "|call setbufvar(".originalBuffer.", \"&wrap\", ".getwinvar(originalWindow, '&wrap').")"
...
" Section: CVS buffer management commands 
...
function! s:CVSVimDiffRestore(vimDiffBuff)
  if exists("g:CVSCommandRestoreVimDiffScratchBuffer") && a:vimDiffBuff == g:CVSCommandRestoreVimDiffScratchBuffer
